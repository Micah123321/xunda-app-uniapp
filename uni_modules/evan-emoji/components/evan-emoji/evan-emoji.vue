<template>
	<view class="evan-emoji">
		<swiper :style="{ height: height }" :indicator-dots="false" :autoplay="false" :current="swiper.activeCurrent"
			@change="fnSwitchEmojiList">
				...
				<scroll-view class="emoji-wrap" :style="{ height: height }" scroll-y="true">
					<view class="emoji-list" :class="[item.type]">
						<view class="emoji-list-item"
							...
							v-for="(emoji, eIndex) in item.list" :key="eIndex" @click="fnSelectEmoji(emoji, item.type)">
							<text v-if="item.type == 'text'" class="emoji-text">{{ emoji.text }}</text>
							<text v-else-if="useHtmlRender && item.type == 'html'"
								class="emoji-html">{{ emoji.html }}</text>
							<cache-image v-else class="emoji-img" :width="60" :height="60" :url="emoji.url"
								:fileMd5="emoji.url" mode="scaleToFill"></cache-image>
						</view>
					</view>
				</scroll-view>
			</swiper-item>
		...
		<view class="control-bar" :class="{ 'no-del-icon': !useDelIcon }">
			<scroll-view class="control-emoji-scroll" scroll-x="true" :scroll-into-view="control.currentId">
				<view :id="'control' + index" class="control-emoji-scroll-item"
					:class="{ actived: swiper.current == index }"
					:style="[{ '--bg-color': activeBgColor, '--radius': activedRadius }]"
					v-for="(item, index) in dataList" @click="fnSwitchEmojiList(index)" :key="item.name">
					{{ item.name }}
				</view>
			</scroll-view>

			<image v-if="useDelIcon" class="control-bar-del-icon" src="../../static/icon/del_icon.png"
				mode="scaleToFill" @click.stop="fnDeleteEmoji()"></image>

			<image v-if="useDelIcon" class="control-bar-submit-icon" src="../../static/icon/submit_icon.png"
				mode="scaleToFill" @click.stop="fnSubmit()" />
