<template>
  <view class="evan-emoji">
        ...
        <scroll-view class="emoji-wrap" :style="{ height: height }" scroll-y="true">
          <view class="emoji-list" :class="[item.type]">
            ...
            >
              ...
              <cache-image v-else class="emoji-img" :width="60" :height="60" :url="emoji.url" :fileMd5="emoji.url"
                           mode="scaleToFill"></cache-image>
            </view>
          </view>
        </scroll-view>
      </swiper-item>
    ...
    <view class="control-bar" :class="{ 'no-del-icon': !useDelIcon }">
      ...
      <image v-if="useDelIcon" class="control-bar-del-icon" src="../../static/icon/del_icon.png" mode="scaleToFill"
             @click.stop="fnDeleteEmoji()"></image>

      <image v-if="useDelIcon" class="control-bar-submit-icon" src="../../static/icon/submit_icon.png" mode="scaleToFill"
           @click.stop="fnSubmit()"/>
